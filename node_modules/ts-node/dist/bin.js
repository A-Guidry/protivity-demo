...
if (isEval) {
    ...
    if (argv._.length) {
        process.argv = ['node --max-old-space-size=10240'].concat(path_1.resolve(cwd, argv._[0])).concat(argv._.slice(1));
...
function replEval(code, _context, _filename, callback) {
    ...
    try {
        ...
        if (error instanceof index_1.TSError) {
            // Support recoverable compilations using >= node --max-old-space-size=10240 6.
